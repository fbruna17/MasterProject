...
class WhateverNet_No_Attention(nn.Module):
    def __init__(self,
                 input_size: int,
                 memory: int,
                 target_size: int,
                 ...
                 dropout: float = 0.1,
                 nr_parameters: int = 1):
        super(WhateverNet_No_Attention, self).__init__()
    ...
    def forward(self, x):
        ...
        # Input: (Batch, Memory, Hidden Size)
        tcn_input = self.pre_tcn_grn(bigru_out)
        tcn_output = self.tcn(tcn_input)

        return tcn_output.view(batch_size, self.memory, 1, self.nr_parameters)


class WhateverNet_No_Bigru(nn.Module):
    def __init__(self,
                 input_size: int,
                 memory: int,
                 target_size: int,
                 ...
                 dropout: float = 0.1,
                 nr_parameters: int = 1):
        super(WhateverNet_No_Bigru, self).__init__()
        ...
        self.bidirection_GRU = GRU(input_size=hidden_size,
                                   hidden_size=hidden_size,
                                   num_layers=bigru_layers,
                                   dropout=dropout)
...
class SolarFlare_No_Attention(TorchParametricProbabilisticForecastingModel, PastCovariatesTorchModel):
    @random_method
    def __init__(
            self,
            input_chunk_length: int,
    ...
    def _create_model(self, train_sample: Tuple[Tensor]) -> torch.nn.Module:
        ...
        output_dim = train_sample[-1].shape[1]
        nr_params = 1 if self.likelihood is None else self.likelihood.num_parameters
        return WhateverNet_No_Attention(input_size=input_dim,
                                        memory=self.input_chunk_length,
                                        horizon=self.output_chunk_length,
                                        target_size=output_dim,
                                        nr_parameters=nr_params,
...
class SolarFlare_No_Bigru(TorchParametricProbabilisticForecastingModel, PastCovariatesTorchModel):
    @random_method
    def __init__(
            self,
            input_chunk_length: int,
    ...
    def _create_model(self, train_sample: Tuple[Tensor]) -> torch.nn.Module:
        ...
        output_dim = train_sample[-1].shape[1]
        nr_params = 1 if self.likelihood is None else self.likelihood.num_parameters
        return WhateverNet_No_Bigru(input_size=input_dim,
                                    memory=self.input_chunk_length,
                                    horizon=self.output_chunk_length,
                                    target_size=output_dim,
                                    nr_parameters=nr_params,
